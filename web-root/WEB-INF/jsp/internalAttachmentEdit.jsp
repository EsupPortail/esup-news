...
<form name="${namespace}InternalAttachmentForm" method="post" 
	...
	<div class="breadcrumb">
		...
		<a href="javascript:;" onclick="submit('<portlet:actionURL><portlet:param name="action" value="editItem"/><portlet:param name="_page" value="${page}"/><c:if test="${not empty topic}"><portlet:param name="tId" value="${topic.topicId}" /></c:if><c:if test="${not empty category}"><portlet:param name="cId" value="${category.categoryId}" /></c:if></portlet:actionURL>');">Toutes les catégories</a>
		 
		<!-- Categorie selectionnee -->
		<c:if test="${not empty selectedCategory}">
			&gt; <a href="javascript:;" onclick="submit('<portlet:actionURL><portlet:param name="action" value="editItem"/><portlet:param name="_page" value="${page}"/><portlet:param name="categoryId" value="${selectedCategory.categoryId}" /><c:if test="${not empty topic}"><portlet:param name="tId" value="${topic.topicId}" /></c:if><c:if test="${not empty category}"><portlet:param name="cId" value="${category.categoryId}" /></c:if></portlet:actionURL>')"><c:out value="${selectedCategory.name}"/></a>
		</c:if>
		
		<!-- Topic selectionne -->
		<c:if test="${not empty selectedTopic}">
			&gt; <a href="javascript:;" onclick="submit('<portlet:actionURL><portlet:param name="action" value="editItem"/><portlet:param name="_page" value="${page}"/><portlet:param name="topicId" value="${selectedTopic.topicId}" /><c:if test="${not empty topic}"><portlet:param name="tId" value="${topic.topicId}" /></c:if><c:if test="${not empty category}"><portlet:param name="cId" value="${category.categoryId}" /></c:if></portlet:actionURL>');"><c:out value="${selectedTopic.name}"/></a>
		</c:if>
		
		<!-- item selectionne -->
		<c:if test="${not empty selectedItem}">
			&gt; <a href="javascript:;" onclick="submit('<portlet:actionURL><portlet:param name="action" value="editItem"/><portlet:param name="_page" value="${page}"/><portlet:param name="itemId" value="${selectedItem.itemId}" /><portlet:param name="selectedTopic" value="${selectedTopic.topicId}" /><c:if test="${not empty topic}"><portlet:param name="tId" value="${topic.topicId}" /></c:if><c:if test="${not empty category}"><portlet:param name="cId" value="${category.categoryId}" /></c:if></portlet:actionURL>');"><c:out value="${selectedItem.title}"/></a>
		</c:if>
	...
	<c:if test="${fn:length(categories) > 0}">
		<table class="internal_attachment_table">
			...
			<tr>
				...
				<td class="internal_attachment_name"><a href="#" onclick="submit('<portlet:actionURL><portlet:param name="action" value="editItem"/><portlet:param name="_page" value="${page}"/><portlet:param name="categoryId" value="${cat.categoryId}"/><c:if test="${not empty topic}"><portlet:param name="tId" value="${topic.topicId}" /></c:if><c:if test="${not empty category}"><portlet:param name="cId" value="${category.categoryId}" /></c:if></portlet:actionURL>');"><c:out value="${cat.name}"/></a></td>
	...
	<c:if test="${fn:length(topics) > 0}">
		<table class="internal_attachment_table">
			...
			<tr>
				<td class="internal_attachment_check"><!-- empty --></td>
				<td class="internal_attachment_type"><img src="<html:imagesPath/>icone_cat_p2.gif"/></td>
				<td class="internal_attachment_name"><a href="#" onclick="submit('<portlet:actionURL><portlet:param name="action" value="editItem"/><portlet:param name="_page" value="${page}"/><portlet:param name="topicId" value="${topicEntry.topicId}"/><c:if test="${not empty topic}"><portlet:param name="tId" value="${topic.topicId}" /></c:if><c:if test="${not empty category}"><portlet:param name="cId" value="${category.categoryId}" /></c:if></portlet:actionURL>');"><c:out value="${topicEntry.name}"/></a>
