...
<p /><c:if test="${not empty itemList}">
	...
	<c:choose>
		<c:when test="${s=='1'}">
			<d:column property="title" titleKey="item.label.title"
				href="${baseRenderUrl}" paramId="${portletParamPrefixe}${pltc_target}iId" paramProperty="itemId" />
			<d:column titleKey="item.label.created.by" href="${userDetailsURL}" paramId="${portletParamPrefixe}${pltc_target}userId"
			...
			<d:column property="endDate" titleKey="item.label.end.time"
				format="{0,date,long}" />
			<d:column titleKey="item.label.updated.by" href="${userDetailsURL}" paramId="${portletParamPrefixe}${pltc_target}userId"
				paramProperty="lastUpdatedBy"><c:choose>				<c:when
			...
			<c:if test='${pMask ge rM}'>
				<d:column href="${upURL}" paramId="${portletParamPrefixe}${pltc_target}iId" paramProperty="itemId" >
				...
				<d:column href="${upTopURL}" paramId="${portletParamPrefixe}${pltc_target}iId" paramProperty="itemId">
					<c:if test="${not (row_rowNum eq 1)}"><img
						title="<fmt:message key='news.img.title.increase.disp.order'/>"
						src="<html:imagesPath/>double_up.gif" border=0 /></c:if>
				</d:column><
				<c:choose>
					<c:when test="${not (row_rowNum eq fn:length(itemList))}"><d:column href="${downURL}" paramId="${portletParamPrefixe}${pltc_target}iId" paramProperty="itemId">
					<img
				...
				<c:choose>
					<c:when test="${not (row_rowNum eq fn:length(itemList))}"><d:column href="${downBottomURL}" paramId="${portletParamPrefixe}${pltc_target}iId" paramProperty="itemId">
					<img
						title="<fmt:message key='news.img.title.decrease.disp.order'/>"
	...
	<c:if test='${pMask ge rM}'>
		...
		<d:column href="${editURL}" paramId="${portletParamPrefixe}${pltc_target}iId" paramProperty="itemId">
			<img title='<fmt:message key="news.img.title.edit.item"/>'
				src="<html:imagesPath/>edit.gif" border=0 />
		</d:column>
		<c:if test="${s!='2'}">
			<d:column href="${apprURL}" paramId="${portletParamPrefixe}${pltc_target}iId" paramProperty="itemId">
				<c:choose>
		...
		<d:column href="${delURL}" paramId="${portletParamPrefixe}${pltc_target}iId" paramProperty="itemId">
		...
		<d:column href="${rDelURL}" paramId="${portletParamPrefixe}${pltc_target}iId" paramProperty="itemId">
