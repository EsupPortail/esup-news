...
<fieldset>
    <legend> <fmt:message key="edit.type.page.title" /> </legend> 
	<html:errors path="typeForm" fields="true" />
	<spring:nestedPath path="typeForm">
<form name="${namespace}editType" method="post"
	action="${editTypeActionURL}">
<table border="0" cellspacing="0" cellpadding="4">
	<tr>
			<td class="portlet-form-label">
				<fmt:message key="news.label.type.name" /> <span class="portlet-msg-alert">*</span>
			</td>
			<td class="portlet-font">
				<html:input path="type.name" size="30" cssClass="portlet-form-input-field" cssStyle="width: 250px;" />
			</td>
	</tr>
	<tr>			
		<td class="portlet-form-label">
			<fmt:message key="news.label.shortDesc" /><span class="portlet-msg-alert">*</span>
		</td>
		<td class="portlet-font">
			<html:textarea path="type.description" cssStyle="width: 250px;"
			rows="8" cols="80" cssClass="portlet-form-input-field" />
		</td>
	</tr>
	<c:if test="${fn:length(entityList) > 0}">
			<tr>
				<td valign="top" class="portlet-form-label"><fmt:message
					key="news.label.type.associated.entities" /><span
					...
					<table border=0 cellpadding=5>
						...
						<tr style="width: 100%;">
							<c:forEach items="${entityList}" var="entityEntry">
								<td><c:forEach items="${typeForm.entitiesIds}" var="eId">
									<c:if test="${eId eq entityEntry.entityId}">
										<c:set var="selected" value="true" />
									</c:if>
								</c:forEach> <input type="checkbox" name="${status.expression}"
									cssClass="portlet-form-input-field"
									<c:if test="${selected}">checked</c:if>
									value="${entityEntry.entityId}" /> <input type="hidden"
