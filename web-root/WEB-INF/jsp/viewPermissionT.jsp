...
<c:out value='${topic.name}' /> </span> </legend>
<div class="texteBasPage">[<a href="${addTPermissionRenderUrl}"
...
<DL id="news_showhide">
	...
	<img src="<html:imagesPath/>help_t.gif" align="absmiddle" alt="" /></DT>
...
<span Style="PADDING-LEFT: 20px;"><img
	src="<html:imagesPath/>ft.gif" alt="" /> <fmt:message
	key="news.label.otherRoles" /> <a href="${viewRolesRenderUrl}" ><img
		src="<html:imagesPath/>help_t.gif" align="absmiddle" title='<fmt:message key="news.label.roles.goToDetails" />' /></a></span>
<p />
<table class="cat">
	<thead>
		<tr>
			...
			<th class="portlet-font"><fmt:message key="news.label.user" /></th>
			...
			<c:when test="${fn:length(mapEntry.value) > 0}">
				<tr>
					...
					<c:forEach items="${mapEntry.value}" var="listItem"
						...
						<c:choose>
							<c:when test="${statusAttrValues.count > 1}">
								<tr>
							</c:when>
						</c:choose>
						<td id="news_showhide" class="portlet-font"><p onClick="slide('news_showhide${role.index}-${statusAttrValues.index}')"><c:choose>
