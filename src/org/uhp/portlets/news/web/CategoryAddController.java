...
public class CategoryAddController extends SimpleFormController {
	...
	/** Logger. */
	private static final Log LOGGER = LogFactory.getLog(EntityAddController.class);
	...
	protected void onSubmitAction(final ActionRequest request, final ActionResponse response,
		...
		if (LOGGER.isTraceEnabled()) {
			LOGGER.trace("Category given in parameters :" + catForm.getCategory());
			LOGGER.trace("List of Type to associate with the Entity :" + catForm.getTypesIds());
		}
		securityCheck(request.getRemoteUser(), NewsConstants.CTX_E, catForm.getCategory().getEntityId());
	...
	protected Object formBackingObject(final PortletRequest request) throws Exception {
		final Long id = PortletRequestUtils.getLongParameter(request, Constants.ATT_ENTITY_ID);
		securityCheck(request.getRemoteUser(), NewsConstants.CTX_E, id);
	...
	protected Map<String, Object> referenceData(final PortletRequest request, final Object command,
		...
		securityCheck(uid, NewsConstants.CTX_E, cat.getEntityId());
	...
	throws Exception {
		if (LOGGER.isTraceEnabled()) {
			LOGGER.trace("Entering init Binder.");
		}
	...
	throws Exception {
		if (LOGGER.isTraceEnabled()) {
			LOGGER.trace("Entering render Invalid Submit.");
		}
		return null;
	}
	...
	throws Exception {
		if (LOGGER.isTraceEnabled()) {
			LOGGER.trace("Entering handle Invalid Submit.");
		}
