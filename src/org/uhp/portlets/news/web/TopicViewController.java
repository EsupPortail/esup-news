 ...
 * 10 mai 2012
 */
public class TopicViewController extends AbstractController implements InitializingBean {
	...
	protected ModelAndView handleRenderRequestInternal(final RenderRequest request, final RenderResponse response) throws Exception {
		...
		if (topic != null) {
			...
			mav.addObject(Constants.ATT_PM, RolePerm.valueOf(
					this.pm.getRoleInCtx(topic.getTopicId(), NewsConstants.CTX_T)).getMask());
	...
	/**
	 * Getter of member nbItemsToShow.
	 * @return <code>String</code> the attribute nbItemsToShow
	 */
	...
	/**
	 * Setter of attribute nbItemsToShow.
	 * @param nbItemsToShow the attribute nbItemsToShow to set
	 */
