...
public class UserRoleDeleteController extends AbstractController implements InitializingBean {
	...
	throws Exception {
		...
		final String principal = request.getParameter(Constants.ATT_USER_ID);
		final boolean isGroup = request.getParameter(Constants.ATT_IS_GRP).equals("1") ? true : false;

		if (!this.rm.isInRoleinCtx(ctxId, ctx, RolePerm.ROLE_MANAGER, request.getRemoteUser(), false)) {
		...
		String role = this.rm.getUserRoleInCtx(ctxId, ctx, principal, isGroup);
		this.rm.removeUserRoleForCtx(principal, isGroup, ctxId, ctx);
		String role2 = this.rm.getUserRoleInCtx(ctxId, ctx, principal, isGroup);
