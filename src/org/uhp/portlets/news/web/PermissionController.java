...
public class PermissionController extends AbstractWizardFormController {
	...
	protected void processFinish(ActionRequest request, ActionResponse response, Object command, BindException errors) throws Exception {
		...
		if (isGrp) {
			Group g = permForm.getGroup();
			this.rm.addSubjectRole(g.getKey(), isGrp, permForm.getRole(), permForm.getCtxType(), permForm.getCtxId());
		} else {
			...
			this.rm.addSubjectRole(permForm.getUser().getUserId(), isGrp, permForm.getRole(), permForm.getCtxType(), permForm.getCtxId());
		}
	...
	protected Map<String, Object> referenceData(PortletRequest request, Object command, Errors errors, int page) throws Exception {
		...
		if (page == 0) {
			...
			model.put(Constants.ATT_ROLE_LIST,  set);
			return model;
		} else if (page == 1) {
			model.put(Constants.CMD_PERM, (PermForm) command);
			if (isGrp) {
				String token = ((PermForm) command).getToken();
			...
			Group g = null;
			if (isGrp) {
				role = this.rm.getUserRoleInCtx(pf.getCtxId(), pf.getCtxType(), pf.getGroup().getKey(), true);
				for (PortalGroup group : groups) {
					if (group.getId().equalsIgnoreCase(pf.getGroup().getKey())) {
						g = new Group();
						g.setKey(group.getId());
						g.setName(group.getName());
						break;
					}
				}
			} else {
				role = this.rm.getUserRoleInCtx(pf.getCtxId(), pf.getCtxType(), pf.getUser().getUserId(), false);
			...
			if (role != null) {
				LOG.warn("Warning : there is already a role attributed to this user/group in this context !");
			...
			model.put(Constants.CMD_PERM, pf);
			model.put(Constants.ATT_LDAP_DISPLAY, um.getLdapUserService().getSearchDisplayedAttributes());
			return model;
