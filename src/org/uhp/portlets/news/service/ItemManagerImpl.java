...
public class ItemManagerImpl implements ItemManager {
	...
	public void updateItemOrderingInTopic(final Long itemId, final Long topicId, boolean up) {
		...
		{
			LOGGER.debug("updateItemOrderingInTopic:: nothing to do ");
			return;
		}
	...
	private Item setItemStartEndDates(Item item) {
		...
		if (item.getStartDate() == null)
		{
			...
			{
				LOGGER.debug("setItemStartDate: item.startDate not set, default startDate to today : " + item.getStartDate());
			}
		}
		if (item.getEndDate() == null)
		{
			...
			if (LOGGER.isDebugEnabled())
			{
				LOGGER.debug("setItemEndDate: item.endDate  not set, default endDate to  startDate + " + nbDaysAfterForExp + " : " + item.getEndDate());
			}
		}
		return item;
	}
