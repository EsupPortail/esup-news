...
public class SimpleLdapEntityServiceImpl extends AbstractLdapService implements LdapEntityService, InitializingBean {
	 ...
	 * A logger.
	 */
	private static final Log LOG = LogFactory.getLog(SimpleLdapEntityServiceImpl.class);
	...
	public void afterPropertiesSet() {
		...
		if (!StringUtils.hasText(dnSubPath)) {
			dnSubPath = null;
			LOG.info(getClass() + ": property dnSubPath is not set"); 
		}
		if (testFilter == null) {
			LOG.info(getClass() + ": property testFilter is not set, target ldap-test will not work."); 
		}
	...
	public void test() {
		if (testFilter == null) {
			LOG.error("can not test the LDAP connection when property testFilter is not set, " 
		...
		try {
			...
			LOG.warn("bad LDAP filter [" + testFilter + "], edit configuration file ldap.xml: " 
					+ e.getCause().getMessage());
			return;
		} catch (LdapConnectionException e) {
			LOG.warn("could not connect to LDAP, edit configuration file ldap.xml: " 
					+ e.getCause().getMessage());
			return;
		} catch (LdapException e) {
			LOG.warn("LDAP error, edit configuration file ldap.xml: " + e.getCause().getMessage());
			return;
		}
		if (ldapEntities.isEmpty()) {
			LOG.warn("no entity retrieved for filter [" + testFilter + "]");
		} else {
			LOG.warn(ldapEntities.size() + " entity(ies) retrieved for filter [" + testFilter + "]:");
			for (LdapEntity ldapEntity : ldapEntities) {
				LOG.warn(" - " + ldapEntity.getId() + ":");
